<template>
  <v-container class="py-8">
    ...
    <template v-else-if="results.length === 2">
      <v-row>
        <v-col v-for="r in results" :key="r.id" cols="12" md="6">
          <v-card elevation="2" rounded="lg" class="pa-4">
            ...
            <div class="d-flex justify-space-between mb-2">
              ...
              <v-chip :color="r.ats_flags?.length ? 'warning' : 'success'" size="x-small" variant="tonal">
            ...
            <div class="d-flex flex-wrap ga-1">
              ...
              <span v-if="!(r.keyword_matches || []).filter(k => !k.found).length" class="text-medium-emphasis">None</span>
            </div>
          </v-card>
        </v-col>
      </v-row>
      ...
      <v-card elevation="2" rounded="lg" class="pa-4 mt-4">
        <div class="text-h6 font-weight-bold mb-3">Score Comparison</div>
